...
LRESULT __stdcall dWndProc(const HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	// ImGui hazır değilse eski WndProc'a gönder
	if (!settings.ImGuiInitialized)
		return CallWindowProc(oWndProc, hWnd, uMsg, wParam, lParam);

	// Resize olursa render target'ı resetle
	...
	// ImGui input'u işlediyse diğer mesajları geç
	...
	// Mesajı orijinal Windows prosedürüne gönder
	return CallWindowProc(oWndProc, hWnd, uMsg, wParam, lParam);
}
...
void DirectX::Shutdown() {
	...
	if (ImGui::GetCurrentContext()) // güvenli kontrol
