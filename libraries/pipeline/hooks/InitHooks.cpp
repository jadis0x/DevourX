...
app::CSteamID dSteamUser_GetSteamID(MethodInfo* method)
{
	...
	return app::SteamUser_GetSteamID(method);
}
...
app::String* dSteamFriends_GetPersonaName(MethodInfo* method)
{
	if(settings.bSpoofSteamName)
	{
		...
		if(newSteamName)
		{
			...
			return app::SteamFriends_GetPersonaName(method);
		}
	}
...
int32_t dPlayerState_get_ConnectionId(app::PlayerState* __this, MethodInfo* method)
{
	uint32_t id = app::PlayerState_get_ConnectionId(__this, method);
	...
	if(Base::DevourNet::IsHost())
	{
		...
		if(settings.bConnectionId)
		{
			uint32_t currentConnId = Base::DevourNet::GetConnectionID();

			if(id == currentConnId)
			{
				app::PlayerState_set_ConnectionId(__this, 0, nullptr);
				return (uint32_t)0;
			}
			else {
				return id;
...
void dPlayerState__ctor(app::PlayerState* __this, MethodInfo* method)
{
	if(Base::DevourNet::IsHost())
	{
		app::PlayerState__ctor(__this, method);
	}
	else {
		if(settings.bConnectionId)
		{
			...
			if(app::PlayerState_get_ConnectionId(__this, nullptr) == currentConnId)
			{
				app::PlayerState_set_ConnectionId(__this, 0, nullptr);
				app::PlayerState__ctor(__this, method);
			}
...
app::String* dPlayerState_get_PlayerId(app::PlayerState* __this, MethodInfo* method)
{
	auto* playerId = app::PlayerState_get_PlayerId(__this, method);

	if(settings.bSpoofSteamId && playerId != nullptr)
	{
		uint64_t spoofedId = strtoull(settings.customSteamIdStr, nullptr, 10);
		app::CSteamID spoof{ spoofedId };
		app::CSteamID__ctor_1(&spoof, spoofedId, nullptr);
		...
		app::String* convertedString = convert_to_system_string(std::to_string(spoof.m_SteamID).c_str());
	...
	else
	{
		if(local_nb == __this)
		{
			...
			if(settings.bFly)
			{
				...
				{
					local_nb = nullptr; // reference nullptr, clean
					return app::NolanBehaviour_Update(__this, method);
				}
		...
		{
			local_nb = nullptr; // another player joined, reset
		}
	}
...
void DetourInitilization()
{
	...
	if(!HookFunction(&(PVOID&)app::PlayerState_get_ConnectionId, dPlayerState_get_ConnectionId, "dPlayerState_get_ConnectionId")) {
		DetourTransactionAbort();
		return;
	}


	if(!HookFunction(&(PVOID&)app::PlayerState__ctor, dPlayerState__ctor, "dPlayerState__ctor")) {
		DetourTransactionAbort();
		return;
	}

	if(!HookFunction(&(PVOID&)app::PlayerState_get_PlayerId, dPlayerState_get_PlayerId, "dPlayerState_get_PlayerId")) {
		DetourTransactionAbort();
		return;
	}
...
void DetourUninitialization()
{
	...
	if(DetourDetach(&(PVOID&)app::PlayerState_get_ConnectionId, dPlayerState_get_ConnectionId) != 0) return;
	if(DetourDetach(&(PVOID&)app::PlayerState__ctor, dPlayerState__ctor) != 0) return;
	if(DetourDetach(&(PVOID&)app::PlayerState_get_PlayerId, dPlayerState_get_PlayerId) != 0) return;
}
