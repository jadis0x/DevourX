...
void NetworkTAB::Render()
{
	...
	ImGui::SetCursorPos(ImVec2(25, 25)); // ← burası padding
	...
	ImGui::NextColumn();
	ImGui::SetNextItemWidth(150);
	ImGui::SliderInt("##LobbySize", &settings.lobbySize, 1, 32);
	...
	ImGui::Checkbox("Connection ID Hook", &settings.bConnectionId);
	ImGui::TextWrapped("Interferes with player's connection ID. Still in testing phase.");
