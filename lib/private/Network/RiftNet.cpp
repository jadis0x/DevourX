...
namespace Rift {
    ...
    // Yapýcý fonksiyon - timeout ve retry ayarlarýný yapýlandýrýr
    RiftNet::RiftNet() : m_timeoutMs(30000), m_proxy(""), m_retryCount(3), m_retryDelayMs(2000) {}

    RiftNet::~RiftNet() {}

    // Timeout ayarlama
    void RiftNet::SetTimeout(short int timeoutMs) {
        this->m_timeoutMs = timeoutMs;
    }

    // Proxy ayarlama
    void RiftNet::SetProxy(const std::string& proxy) {
        m_proxy = proxy;
    }

    // Retry sayýsý ayarlama
    void RiftNet::SetRetryCount(int retryCount) {
        m_retryCount = retryCount;
    }

    // Retry gecikme süresi ayarlama
    void RiftNet::SetRetryDelay(int retryDelayMs) {
        m_retryDelayMs = retryDelayMs;
    }

    // URL'yi parse eden fonksiyon
    bool RiftNet::ParseURL(const std::string& url, std::string& host, std::string& path, short& port, bool& isSecure) {
    ...
    bool RiftNet::ValidateResponseCert(HINTERNET hRequest) {
        DWORD flags = 0;
    ...
    std::string RiftNet::UrlEncode(const std::string& value) {
    ...
    // Query string inþasý
    std::string RiftNet::BuildQueryString(const std::map<std::string, std::string>& params) {
    ...
    std::string RiftNet::PrepareHeaders(const std::map<std::string, std::string>& headers) {
        std::ostringstream oss;
    ...
    // Ýstek detaylarýný loglama
    void RiftNet::LogRequestDetails(
        HttpMethod method,
    ...
    // Yanýt detaylarýný loglama
    void RiftNet::LogResponseDetails(const HttpResponse& response) {
    ...
    bool RiftNet::SendHTTPRequest(
        ...
        // Ýstek bayraklarý - HTTPS için secure flag ekleme
        DWORD flags = INTERNET_FLAG_RELOAD;
        if (isSecure) {
            flags |= INTERNET_FLAG_SECURE;
        }

        // HTTP isteði oluþturma
        ...
        // POST veya PUT istekleri için body ekle
        ...
        while (InternetReadFile(hRequest, buffer, sizeof(buffer) - 1, &bytesRead) && bytesRead != 0) {
            buffer[bytesRead] = '\0';  // String'in sonunu belirtmek için null karakter eklenir
            response.body += buffer;  // Yanýt gövdesi oluþturulur
        }

        // Yanýt kodunu ve baþlýklarý kontrol et
        DWORD statusCode = 0;
        DWORD statusCodeSize = sizeof(statusCode);
        if (HttpQueryInfo(hRequest, HTTP_QUERY_STATUS_CODE | HTTP_QUERY_FLAG_NUMBER, &statusCode, &statusCodeSize, NULL)) {
            response.statusCode = statusCode;  // HTTP yanýt kodu
        }

        // Yanýt baþlýklarýný oku
